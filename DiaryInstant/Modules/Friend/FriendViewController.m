...
@interface FriendViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>{
...
-(void)makeTable{
    ...
    [add mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(_headImageView.mas_left).offset(5);
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section ==0){
        ...
        tisLabel.text = list[indexPath.row];//oneArr[indexPath.row];
...
-(void) readMessage{
    ...
    [manager POST:@"http://di.leizhenxd.com/api/user/getMyFriends" parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([[jsonDict[@"responseCode"] stringValue] isEqualToString:@"0"]){
            ...
            for(NSDictionary *item in imDic){
                [list addObject:[NSString stringWithFormat:@"%@",item[@"name"]]];
            }
